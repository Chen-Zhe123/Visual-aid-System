...
public class ConnectService extends Service {
    private String TAG = ConnectService.class.getSimpleName();
    ...
    class ControllerBinder extends Binder {
        public ConnectService getBridgeService() {
...
//                .setContentTitle(getResources().getString(R.string.app_name))
                .setContentTitle("已开启无线摄像头P2P服务")
//                .setContentText("点击可关闭服务") // 设置上下文内容
                .setWhen(System.currentTimeMillis()); // 设置该通知发生的时间

        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            //修改安卓8.1以上系统报错
            NotificationChannel notificationChannel = new NotificationChannel("2222", "2222", NotificationManager.IMPORTANCE_MIN);
            notificationChannel.setShowBadge(false);//是否显示角标
            notificationChannel.setLockscreenVisibility(Notification.VISIBILITY_SECRET);
            NotificationManager manager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
            manager.createNotificationChannel(notificationChannel);
            builder.setChannelId("2222");
        }
    ...
    /**
     * PlayVideoActivity feedback method
     * <p>
     * jni
     *
     * @param did      设备唯一ID
     * @param videobuf ⼀帧视频数据
     * @param h264Data 0 普清摄像机数据，1 ⾼清摄像机数据
     * @param len      ⼀帧数据⼤⼩
     * @param width    ⼀帧数据宽
     * @param height   ⼀帧数据⾼
     */
    ...
    public void VideoData(String did, byte[] videobuf, int h264Data, int len,
        ...
        Log.d(TAG, "ConnectService----Call VideoData 视频数据返回...h264Data: "
        ...
        if (serviceInterface != null && videobuf.length > 0 && len > 0) {
            serviceInterface.videoDataCallback(videobuf, h264Data, len, width, height);
        }
    }
...
//	}
	...
	private Notification getNotification(String content, String did,boolean isAlarm)
	{
		*//*mNotify2 = new Notification(R.drawable.app,content, System.currentTimeMillis());
		...
		mNotify2.setLatestEventInfo(ConnectService.this, "This is content title",
    ...
    public void CallBackTransferMessage(String did, String resultPbuf, int cmd,
        ...
        if (cmd == ContentCommon.CGI_IEGET_FACTORY) {
            if (resultPbuf.contains("correctModel=")) {
                ...
                SharedPreferenceUtil.saveDeviceInformation(this, did, ContentCommon.DEVICE_MODEL_TYPE, correctModel);
            }
        }
    ...
    public void CallBack_HardFrameData(String did, byte[] pData, int nDataLen,
                                       int isH265, int IsIDR, int videoWidth, int videoHeight,
                                       int timestamp, int milistamp, float sessid,
    ...
    // my modification
    private static FaceDetectServiceInterface serviceInterface;

    public static void setServiceInterface(FaceDetectServiceInterface pInterface) {
        serviceInterface = pInterface;
    }

    public interface FaceDetectServiceInterface {

        void videoDataCallback(byte[] videobuf, int h264Data, int len,
                               int width, int height);
    }
}
